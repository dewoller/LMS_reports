...
if (private)  {
 ...
 log %>%
   ...
   filter(semester_week>0) %>%
    ...
    ggplot( aes( semester_week, n, fill=lab)) +
    geom_col() +
    facet_wrap( ~user_full_name, ncol=3) 
...
log %>%
  mutate( in_lab =ifelse( lab=='None', 0,1)) %>%
  distinct( subject_code, user_full_name, student_id_final_digit , semester_week, in_lab ) %>% # do students access in a week i/o
  filter(semester_week>0) %>%
  count( subject_code,  semester_week, in_lab ) %>%
  group_by( semester_week, subject_code ) %>%
  mutate( pct = n/sum(n)*100) %>% 
  ungroup() %>%
  ggplot(aes(as.factor(semester_week), pct, fill=subject_code, group=subject_code)) + 
